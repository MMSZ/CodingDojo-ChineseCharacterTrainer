...
' $1="$2$3$4"').match(/[cC][lL][aA][sS][sS]=\"[^\"]*\bnocode\b/)){var y=e.match(P)[2],u=1,w;w=g+1;e:for(;w<v;++w){var m=k[w].match(P);if(m&&m[2]===y)if("/"===m[1]){if(0===--u)break e}else++u}w<v?(h.push(t,k.slice(g,w+1).join("")),g=w):h.push(t,e)}else h.push(t,e)}else{var d;var u=e,s=u.indexOf("&");if(0>s)d=u;else{for(--s;0<=(s=u.indexOf("&#",s+1));){var f=u.indexOf(";",s);if(0<=f){var q=u.substring(s+3,f),l=10;q&&"x"===q.charAt(0)&&(q=q.substring(1),l=16);var z=parseInt(q,l);isNaN(z)||(u=u.substring(0,
